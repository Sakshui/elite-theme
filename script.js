...
function renderSupportStats(tickets) {
  ...
  statsContainer.innerHTML = `
    ${statCard("Open Tickets", counts.open, "#2563eb")}
    ${statCard("In Progress", counts["in progress"], "#7c3aed")}
    ${statCard("Resolved", counts.resolved, "#16a34a")}
...
document.addEventListener("click", function(e) {
  ...
  const item = e.target.closest("[data-menu] [data-value]");
  ...
  ">
    ...
    ">
      ...
      ">
        ...
        ">
          <div>
            <div style="color:#6b7280;margin-bottom:4px;">Ticket ID</div>
            <div id="conversationTicketId" style="font-weight:600;"></div>
          </div>

          <div>
            <div style="color:#6b7280;margin-bottom:4px;">Ticket Type</div>
          ...
          <div>
            <div style="color:#6b7280;margin-bottom:4px;">Priority</div>
          ...
          <div>
            ...
            <div id="conversationCreatedAt" style="font-weight:600;"></div>
          </div>
        </div>
      </div>

      <!-- CONVERSATION THREAD -->
      <div id="conversationThread" style="margin-bottom:18px;">
        ...
        <div style="display:flex;justify-content:flex-end;margin-bottom:14px;">
          <div style="
            ...
            padding:12px;
          ">
            <div style="font-size:13px;margin-bottom:4px;text-align:left;">
              <span style="font-weight:700;">You</span>
              <span id="conversationCustomerDate" style="margin-left:8px;color:#dbeafe;"></span>
            </div>
            <div id="conversationCustomerMessage" style="
              font-size:13px;
              line-height:1.4;
              text-align:left;
            "></div>
        ...
        <div style="display:flex;justify-content:flex-start;margin-bottom:18px;">
          ...
          ">
            <div style="font-size:13px;margin-bottom:4px;text-align:left;">
              <span style="font-weight:700;">Support Team</span>
              <span id="conversationSupportDate" style="margin-left:8px;color:#6b7280;"></span>
            </div>
            <div id="conversationSupportMessage" style="
              font-size:13px;
              line-height:1.4;
              text-align:left;
            "></div>
...
function attachSupportTicketsLogic() {
  ...
  const statusBtn = document.getElementById("statusDropdownBtn") || document.querySelector("[data-dropdown='statusDropdown']");
  const priorityBtn = document.getElementById("priorityDropdownBtn") || document.querySelector("[data-dropdown='priorityDropdown']");
  const statusMenu = document.getElementById("statusDropdownMenu") || document.querySelector("[data-menu='statusDropdown']");
  const priorityMenu = document.getElementById("priorityDropdownMenu") || document.querySelector("[data-menu='priorityDropdown']");
  ...
  [statusMenu, priorityMenu].forEach(menu => {
    if (!menu) return;
    menu.querySelectorAll(".dd-item, [data-value]").forEach(item => {
...
function attachTicketConversationModalLogic() {
  ...
  container.addEventListener("click", (event) => {
    ...
    if (!card) return;
    if (event.target.closest(".close-ticket-btn")) return;

    const ticketId = Number(card.querySelector(".close-ticket-btn")?.dataset.ticketId || card.dataset.id);
    const ticket = supportTicketState.tickets.find(t => Number(t.id) === ticketId);
    if (!ticket) return;
    ...
    document.getElementById("conversationTitle").textContent =
      ticket.subject || "Ticket Conversation";

    document.getElementById("conversationTicketId").textContent =
      ticket.support_ticket_id || ticket.id;

    document.getElementById("conversationCreatedAt").textContent = createdAt;

    document.getElementById("conversationCustomerDate").textContent = createdAt;
    document.getElementById("conversationSupportDate").textContent = createdAt;

    document.getElementById("conversationCustomerMessage").textContent =
      ticket.description || "No message available.";

    document.getElementById("conversationSupportMessage").textContent =
      "Thank you for contacting support. Our team is reviewing your ticket.";
    ...
    `;
    statusEl.textContent = toTitleCase(ticket.status);

    // Priority Badge
    const priorityEl = document.getElementById("conversationPriority");
    ...
    `;
    priorityEl.textContent = toTitleCase(ticket.priority);

    // Type badge stays styled from HTML
  ...
  modal.onclick = (e) => {
    ...
    if (replyInput && sendBtn) {
      ...
      const syncSendState = () => {
        ...
        sendBtn.style.background = hasText ? "#2563eb" : "#93c5fd";
        sendBtn.style.cursor = hasText ? "pointer" : "not-allowed";
        sendBtn.style.opacity = hasText ? "1" : "0.85";
      };
  
      replyInput.addEventListener("input", syncSendState);
...
function attachDropdownLogic() {
  document.querySelectorAll("[id$='DropdownBtn'], [data-dropdown]").forEach(btn => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const menu = btn.dataset.dropdown
        ? document.querySelector(`[data-menu='${btn.dataset.dropdown}']`)
        : document.getElementById(btn.id.replace("Btn", "Menu"));
      if (!menu) return;
      const isOpen = menu.style.display === "block";
      document.querySelectorAll("[id$='DropdownMenu'], [data-menu]").forEach(m => {
  ...
  document.querySelectorAll(".dd-item, [data-value]").forEach(item => {
    item.onclick = (e) => {
      e.stopPropagation();
      const menu = item.closest("[data-menu]") || item.parentElement;
      if (!menu) return;

      const btn = menu.dataset.menu
        ? document.querySelector(`[data-dropdown='${menu.dataset.menu}']`)
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest("[id$='DropdownBtn'], [data-dropdown], [id$='DropdownMenu'], [data-menu]")) {
      document.querySelectorAll("[id$='DropdownMenu'], [data-menu]").forEach(m => {
        m.style.display = "none";
      });
    }
...
/* ---------- Support ticket API + order-by-issue enhancements ---------- */
...
async function closeSupportTicket(ticketId) {
  ...
  try {
    ...
    supportTicketState.tickets = (supportTicketState.tickets || []).map((ticket) =>
      Number(ticket?.id) === normalizedId ? { ...ticket, status: "closed" } : ticket
    );
...
function renderSupportTicketList() {
  ...
  ticketList.innerHTML = tickets.map((ticket) => mapApiTicketToCard(ticket)).join("");
...
function attachCloseTicketLogic() {
  document.querySelectorAll(".close-ticket-btn").forEach((button) => {
    button.onclick = async (event) => {
      event.stopPropagation();
      await closeSupportTicket(button.dataset.ticketId);
    };
  });
}

function getSupportTicketOrderOptions() {
  const orderSources = [window.customerOrders, window.orders, window.ordersData, window.customer_orders];
  const rawOrders = orderSources.find((source) => Array.isArray(source)) || [];

  return rawOrders
    .map((order) => {
      const value = order?.orderId || order?.id || order?.order_id || "";
      const label = order?.orderNumber || order?.name || order?.order_name || value;
      if (!value || !label) return null;
      return { value: String(value), label: String(label), order };
    })
    .filter(Boolean);
}

function attachRelatedOrderByIssueLogic() {
  const issueSelect = document.getElementById("ticketIssueInput") || document.getElementById("ticketTypeInput");
  const relatedOrderField = document.getElementById("relatedOrderField");

  if (!issueSelect || !relatedOrderField) return;

  const issuesRequiringOrder = new Set(["order issue", "order enquiry", "return issue", "return request", "refund issue", "refund request", "delivery issue"]);

  const toggle = () => {
    const selectedText = issueSelect.options?.[issueSelect.selectedIndex]?.textContent?.toLowerCase()?.trim() || issueSelect.value?.toLowerCase()?.trim() || "";
    const shouldShow = issuesRequiringOrder.has(selectedText);
    relatedOrderField.style.display = shouldShow ? "block" : "none";

    if (!shouldShow) {
      const input = document.getElementById("ticketOrderInput");
...
function attachTicketOrderSelectLogic() {
  ...
  if (options.length === 0) {
    ...
    options.forEach((option) => {
      ...
      optionBtn.style.cssText = "width:100%;border:none;background:transparent;padding:10px 12px;cursor:pointer;text-align:left;";
...
function createSupportTicket() {
  const ticketType = document.getElementById("ticketTypeInput")?.value?.trim() || "General";
  ...
  if (!subject || !description) {
    createStatusToast("Missing Fields", "Subject and description are required.", "error");
    return;
  }
  ...
  const payload = {
    ...
    customer_details: {
      ...
      customer_first_name: typeof profile_first_name !== "undefined" ? profile_first_name : "",
      customer_last_name: typeof profile_last_name !== "undefined" ? profile_last_name : "",
      customer_email: email || "",
      customer_phone: typeof customer_phone !== "undefined" ? customer_phone : "",
    },
...
function ticketCard({ id, title, tag, desc, status, priority, meta, date, isUpdating }) {
  ...
  return `
  <div class="support-ticket-card" data-search="${`${title} ${desc} ${meta}`.toLowerCase()}" data-status="${statusKey}" data-priority="${String(priority || "").toLowerCase()}" style="border:1px solid #e5e7eb;border-radius:10px;padding:16px;cursor:pointer;transition:all .2s ease;" onmouseover="this.style.boxShadow='0 4px 14px rgba(0,0,0,.08)'" onmouseout="this.style.boxShadow='none'">
    <div style="display:flex;justify-content:space-between;gap:16px;">
      <div style="flex:1;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;"><strong style="font-size:16px;">${title}</strong><span style="font-size:12px;background:#eef2ff;color:#3730a3;padding:2px 8px;border-radius:999px;font-weight:600;">${tag}</span></div>
        <div style="font-size:14px;color:#4b5563;margin-bottom:6px;">${desc}</div>
        <div style="font-size:13px;color:#6b7280;">${meta}</div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;">
        ${isClosed ? "" : `<button class="close-ticket-btn" data-ticket-id="${id || ""}" ${isUpdating ? "disabled" : ""} style="border:1px solid #d1d5db;background:#fff;color:#1f2937;border-radius:999px;padding:2px 10px;font-size:12px;font-weight:600;cursor:${isUpdating ? "not-allowed" : "pointer"};opacity:${isUpdating ? "0.6" : "1"};">${isUpdating ? "Closing..." : "Close"}</button>`}
        <span style="font-size:12px;padding:2px 10px;border-radius:999px;font-weight:600;${statusStyles[statusKey] || ""}">${status}</span>
        <span style="font-size:12px;padding:2px 10px;border-radius:999px;font-weight:600;${priorityStyles[String(priority || "").toLowerCase()] || ""}">${priority}</span>
...
function createTicketModal() {
  ...
  const priorityOptionRow = (value) => `
    <button type="button" class="priority-option-item" data-value="${value}" style="width:100%;display:flex;align-items:center;gap:8px;border:none;background:transparent;padding:6px 10px;cursor:pointer;text-align:left;">
      <span class="priority-option-check" style="width:14px;color:#111827;font-size:14px;visibility:hidden;">✓</span>
      <span style="display:inline-flex;align-items:center;padding:2px 10px;border-radius:999px;font-size:15px;line-height:1;font-weight:600;${priorityBadgeStyles[value]}">${value}</span>
    </button>`;

  return `
  <div id="ticketModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);justify-content:center;align-items:center;z-index:1000;">
    <div style="background:#fff;border-radius:10px;width:420px;padding:20px;position:relative;">
      ...
      <div id="relatedOrderField" style="display:none;margin-bottom:10px;">
        <label style="display:block; margin-bottom:4px;text-align: left;">Related Order</label>
        <div id="ticketOrderSelect" style="position:relative;">
          <button type="button" id="ticketOrderSelectBtn" style="width:100%;min-height:42px;padding:6px 10px;border:1px solid #d1d5db;border-radius:10px;display:flex;justify-content:space-between;align-items:center;background:#fff;cursor:pointer;"><span id="ticketOrderSelectLabel" style="color:#6b7280;">Select an order</span><span style="color:#6b7280;font-size:16px;line-height:1;">▾</span></button>
          <div id="ticketOrderSelectMenu" style="display:none;position:absolute;left:0;top:calc(100% + 6px);width:100%;max-height:180px;overflow:auto;background:#fff;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 10px 24px rgba(15,23,42,.12);z-index:20;padding:4px 0;"></div>
          <input type="hidden" id="ticketOrderInput" value="" />
        </div>
      </div>
      ...
      <label style="display:block; margin-bottom:4px;text-align: left;">Priority</label>
      <div id="prioritySelect" style="position:relative;margin-bottom:10px;">
        <button type="button" id="prioritySelectBtn" style="width:100%;min-height:42px;padding:6px 10px;border:1px solid #d1d5db;border-radius:10px;display:flex;justify-content:space-between;align-items:center;background:#fff;cursor:pointer;"><span id="prioritySelectLabel" style="display:inline-flex;align-items:center;padding:2px 10px;border-radius:999px;font-size:15px;line-height:1;font-weight:600;${priorityBadgeStyles.Medium}">Medium</span><span style="color:#6b7280;font-size:16px;line-height:1;">▾</span></button>
      ...
      <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:16px;"><button id="closeModal">Cancel</button><button id="createTicketSubmitBtn" style="background:#2563eb;color:#fff;border:none;padding:8px 14px;border-radius:6px;">+ Create Ticket</button></div>
    </div>
  </div>`;
}
...
function renderSupportTickets(container) {
  container.innerHTML = `
  <div style="max-width:1200px;margin:0 auto;padding:24px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);background:#fff;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;"><div><h2 style="margin:0 0 6px;font-size:20px;font-weight:700;">My Support Tickets</h2><p style="margin:0;color:#6b7280;font-size:14px;">Track and manage your support requests</p></div><button id="openCreateTicket" style="background:#2563eb;color:#fff;border:none;border-radius:6px;padding:10px 16px;font-size:14px;font-weight:500;cursor:pointer;">+ Create New Ticket</button></div>
    <div id="supportTicketStats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px;"></div>
    <div style="display:flex;gap:12px;margin-bottom:24px;position:relative;"><input id="supportTicketSearch" placeholder="Search tickets by subject, number, or description..." style="flex:1;padding:10px 12px;border-radius:6px;border:1px solid #d1d5db;font-size:14px;" />
      ${dropdown("statusDropdown", "All Status", ["All Status","Open","In Progress","Pending","Resolved","Closed"])}
      ${dropdown("priorityDropdown", "All Priority", ["All Priority","Low","Medium","High","Critical","Urgent"])}
    </div>
    <div id="supportTicketList" style="display:flex;flex-direction:column;gap:16px;"></div>
    <p id="supportTicketEmptyState" style="display:none;color:#6b7280;text-align:center;margin-top:12px;">No tickets found.</p>
    ${createTicketModal()}
    ${ticketConversationModal()}
  </div>`;
...
function ensureSupportTicketsContainer() {
  ...
  if (!container) {
    ...
    container.style.display = "none";
    const tabContent = document.getElementById("tabContent");
    if (tabContent) tabContent.appendChild(container);
...
function attachModalLogic() {
  ...
  if (submitBtn) {
    submitBtn.onclick = createSupportTicket;
  }
...
/* ---------- Support ticket taxonomy + modal/payload alignment fixes ---------- */
...
async function loadIssueTypes() {
  ...
  try {
    ...
    const response = await fetch(`${getSupportApiUrl(ticketTaxonomyApi.issues)}?${query}`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
...
function loadCategories(issueId) {
  const categorySelect = document.getElementById("ticketCategoryInput");
  const subCategorySelect = document.getElementById("ticketSubCategoryInput");
  ...
  const selectedIssue = ticketTaxonomyState.issues.find((issue) => String(issue.issue_id) === String(issueId));
...
function loadSubCategories(categoryId) {
  const issueId = document.getElementById("ticketIssueInput")?.value;
  const subCategorySelect = document.getElementById("ticketSubCategoryInput");
  ...
  const selectedIssue = ticketTaxonomyState.issues.find((issue) => String(issue.issue_id) === String(issueId));
  const selectedCategory = (selectedIssue?.categories || []).find((category) => String(category.category_id) === String(categoryId));
  const subCategories = selectedCategory?.sub_categories || [];

  subCategories.forEach((subCategory) => {
    const option = document.createElement("option");
    option.value = String(subCategory.sub_category_id);
  ...
  <div id="ticketModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);justify-content:center;align-items:center;z-index:1000;">
    <div style="background:#fff;border-radius:10px;width:420px;padding:20px;position:relative;">
      ...
      <h3 style="margin:0 0 12px;text-align:left;font-family:sans-serif;display:flex;align-items:center;">
        <svg style="color:#007bff;margin-right:10px;" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
        Create New Support Ticket
      </h3>

      <label style="display:block;margin-bottom:4px;text-align:left;">Issue Type <span style="color:red;">*</span></label>
      <select id="ticketIssueInput" style="width:100%;padding:8px;margin-bottom:10px;">
        <option value="">Loading issues...</option>
      </select>

      <label style="display:block;margin-bottom:4px;text-align:left;">Category <span style="color:red;">*</span></label>
      <select id="ticketCategoryInput" style="width:100%;padding:8px;margin-bottom:10px;" disabled>
        <option value="">Select category</option>
      </select>

      <label style="display:block;margin-bottom:4px;text-align:left;">Sub Category <span style="color:red;">*</span></label>
      ...
      <div id="relatedOrderField" style="display:none;margin-bottom:10px;">
        <label style="display:block; margin-bottom:4px;text-align:left;">Related Order <span style="color:red;">*</span></label>
        <div id="ticketOrderSelect" style="position:relative;">
          <button type="button" role="combobox" aria-expanded="false" aria-autocomplete="none" id="ticketOrderSelectBtn" style="width:100%;min-height:42px;padding:6px 10px;border:1px solid #d1d5db;border-radius:10px;display:flex;justify-content:space-between;align-items:center;background:#fff;cursor:pointer;">
            <span id="ticketOrderSelectLabel" style="color:#6b7280;">Select an order</span>
            <span style="color:#6b7280;font-size:16px;line-height:1;">▾</span>
          </button>
          <div id="ticketOrderSelectMenu" style="display:none;position:absolute;left:0;top:calc(100% + 6px);width:100%;max-height:180px;overflow:auto;background:#fff;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 10px 24px rgba(15,23,42,.12);z-index:20;padding:4px 0;"></div>
      ...
      <label style="display:block;margin-bottom:4px;text-align:left;">Priority</label>
      <div id="prioritySelect" style="position:relative;margin-bottom:10px;">
        <button type="button" id="prioritySelectBtn" style="width:100%;min-height:42px;padding:6px 10px;border:1px solid #d1d5db;border-radius:10px;display:flex;justify-content:space-between;align-items:center;background:#fff;cursor:pointer;">
          <span id="prioritySelectLabel" style="display:inline-flex;align-items:center;padding:2px 10px;border-radius:999px;font-size:15px;line-height:1;font-weight:600;${priorityBadgeStyles.Medium}">Medium</span>
          <span style="color:#6b7280;font-size:16px;line-height:1;">▾</span>
        </button>
        <div id="prioritySelectMenu" style="display:none;position:absolute;left:0;top:calc(100% + 6px);width:100%;background:#fff;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 10px 24px rgba(15,23,42,.12);z-index:20;padding:4px 0;">
      ...
      <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:16px;">
        <button id="closeModal">Cancel</button>
        <button id="createTicketSubmitBtn" style="background:#2563eb;color:#fff;border:none;padding:8px 14px;border-radius:6px;">+ Create Ticket</button>
      </div>
    </div>
  </div>`;
}
...
function createSupportTicket() {
  ...
  if (!issueId || !categoryId || !subCategoryId || !subject || !description) {
    createStatusToast("Missing Fields", "Issue, category, sub category, subject and description are required.", "error");
    return;
  }
